        ...
        if properties:
            ...
            if 108 <= (cell_area * (pixel_size_um ** 2)) <= 5400 and (cell_major_axis_length * pixel_size_um) <= 32.4:
                ...
                for time_point in range(1, 34):
                    ...
                    # Create a mask eroded by 1 pixel (for boundary)
                    eroded_mask_1 = binary_erosion(cropped_mask, disk(1))
                    
                    # Create a mask eroded by 2 pixels (to get just 1 pixel in from boundary)
                    eroded_mask_2 = binary_erosion(cropped_mask, disk(2))
                    
                    # Get boundary pixels
                    boundary = cropped_mask & ~eroded_mask_1
                    
                    # Get the 1-pixel ring just inside the boundary
                    inner_ring = eroded_mask_1 & ~eroded_mask_2
                    
                    # Combine boundary and inner ring to get the membrane zone
